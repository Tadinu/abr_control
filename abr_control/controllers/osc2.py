...
class OSC2():
    """
        OSC provides Operational Space Control for a given BaseRobot.
    ...
    def __limit_vel(self, u_task: np.ndarray, device_model: DeviceModel):
        """
        ...
        if device_model.max_vel is not None:
            ...
            sat_gain_xyz = device_model.max_vel[0] / kp * kv
            scale_xyz = device_model.max_vel[0] / kp * kv
            ...
            sat_gain_abg = device_model.max_vel[1] / ko * kv
            scale_abg = device_model.max_vel[1] / ko * kv
            if norm_abg > sat_gain_abg:
                scale[3:] *= scale_abg / norm_abg
            u_task = kv * scale * lamb * u_task
        else:
            print("DeviceModel max_vel must be set in the yaml file!")
            raise Exception

        return u_task

    def calc_error(self, target: Target, device_model: DeviceModel):
        """
            Compute the difference between the target and EE
            for the x,y,z and a,b,g components
        """
        u_task = np.zeros(6)
        # Calculate x,y,z error
        if np.sum(device_model.ctrlr_dof_xyz) > 0:
            diff = device_model.get_state(DeviceState.EE_XYZ) - target.get_xyz()
            u_task[:3] = diff
