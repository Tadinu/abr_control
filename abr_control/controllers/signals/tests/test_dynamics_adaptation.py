...
def test_scaling():
    robot_model = ur5.Config(use_cython=True)

    # looking at data centered around 50 with range of 25 each way
    means = np.ones(robot_model.N_JOINTS) * 50
    variances = np.ones(robot_model.N_JOINTS) * 25

    for ii in range(50):
        # test without spherical conversion
        adapt = DynamicsAdaptation(
            robot_model.N_JOINTS,
            robot_model.N_JOINTS,
            means=means,
            variances=variances,
            spherical=False,
        ...
        unscaled = np.random.random(robot_model.N_JOINTS) * 50 + 25
