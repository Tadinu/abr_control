...
import numpy as np
from abr_control.controllers import OSC, OSC2
from abr_control.app import MujocoApp
from abr_control.arms import DeviceState
from abr_control.controllers.utils import Target, ControllerConfig
...
class InsertionTask(MujocoApp):
    ...
    """
    def __init__(self, app_config_file: str = None, scene_file: str = None, active_arm_model_name: str = 'ur5right'):
        # Initialize the Parent class with the config file
        super().__init__(app_config_file, scene_file)
        # Specify the robot in the scene that we'd like to use
        self.robot = self.get_robot(robot_name="DualUR5")
        
        self.arm_model_name_list = ['ur5right', 'ur5left']
        self.arm_model_list = [self.robot.get_device_model(arm_model_name) for arm_model_name in self.arm_model_name_list]
        self.set_active_arm(active_arm_model_name)
        ...
        self.controller = OSC2(self.robot, self.sim, osc_device_configs, nullspace_config)
        ...
        #self.viewer = mujoco.viewer.launch_passive(self.model_ptr, self.data_ptr, key_callback=self.key_callback)
        ## Set the camera distance/angle
        assert self.viewer.camera
        self.viewer.camera.azimuth = -90
        self.viewer.camera.elevation = -40
        self.viewer.camera.distance = self.model_ptr.stat.extent
    ...
    def set_active_arm(self, active_arm_model_name):
        """
        ...
        """
        # Set the ur5 active & passive arm from [active_arm_model_name]
        for arm_model in self.arm_model_list:
            if arm_model.name == active_arm_model_name:
                self.active_arm = arm_model
            else:
                self.passive_arm = arm_model
    ...
    def get_action_config(self, config_file: str):
        ...
        action_obj_config_path = os.path.join(self.main_dir, config_file)
            ...
            if type(update_errors) == list:
                for device_name in update_errors:
                    ...
                    self.errors[device_name] = np.linalg.norm(
                        self.controller.calc_error(self.targets[device_name], self.robot.get_device_model(device_name)))
    ...
    def set_waypoint_targets(self, params):
        ...
        if 'target_xyz' in params.keys():
            ...
            if isinstance(params['target_xyz'], str):
                ...
                if params['target_xyz'] == 'start_pos':
                    ...
                    target_joint = self.action_objects[params['target_xyz']]['joint_name']
                    target_joint_id = self.sim_model.name2id(target_joint, 'joint')
                    target_joint_pos_addrs = [self.model_ptr.jnt_qposadr[target_joint_id]]
                    obj_pos = self.data_ptr.qpos[target_joint_pos_addrs][:3]
        ...
        if 'target_abg' in params.keys():
            if isinstance(params['target_abg'], str):
                ...
                target_joint_id = self.sim_model.name2id(target_obj['joint_name'], 'joint')
                obj_quat = self.get_orientation_by_id(target_joint_id, 'body')
    ...
    def initialize_action_objects_random(self, arm_model_name):
        """
        ...
        male_obj['initial_pos_xyz'][0] = mx if arm_model_name == 'ur5right' else -1*mx
        ...
        female_obj['initial_pos_xyz'][0] = fx if arm_model_name == 'ur5right' else -1*fx
    ...
    def run(self, robot_name, device_model_name_list, randomize = False):
        ...
        action_config_name = 'arms/dual_ur5/configs/insertion_task.yaml'
        ...
        if randomize:
            ...
            for arm_idx, obj_idx in zip(arm_idxs, obj_idxs):
                ...
                self.set_active_arm(self.arm_model_name_list[arm_idx])
                self.initialize_action_objects_random(self.arm_model_name_list[arm_idx])
        ...
        # If we want to run the demo with object locations specified by the configuration file
            ...
            for arm_model_name, action_object_name in zip(self.arm_model_name_list, action_object_names):
                # Initialize the action objects and active arm
                action_config = self.get_action_config(action_config_name)
                self.action_objects = action_config[action_object_name]
                self.set_active_arm(arm_model_name)
        ...
        super().run( robot_name, device_model_name_list, randomize)

# Main entrypoint
if __name__ == "__main__":
    # Initialize the insertion Task
    demo = InsertionTask(app_config_file="arms/dual_ur5/configs/default_xyz_abg.yaml",
                         scene_file="arms/dual_ur5/scenes/insertion_task_scene.xml", active_arm_model_name='ur5right')
    # Run the non-randomized demo (user-specified object locations/orientations)
    demo.run('DualUR5', device_model_name_list=['ur5right', 'ur5left'], randomize=False)
    # Run the randomized demo (randomly generated object locations/orientations)
    demo.run('DualUR5', device_model_name_list=['ur5right', 'ur5left'], randomize=True)